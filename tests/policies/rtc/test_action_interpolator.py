...
def test_queue_interpolator_merge_resets_queue_but_interpolator_keeps_prev():
    ...
    assert first_from_new.item() == pytest.approx(6.0)
...
def test_queue_interpolator_delay_skips_stale_actions():
    ...
    torch.testing.assert_close(leftover[0], torch.tensor([103.0, 103.0]))

    first_action = queue.get()
    assert first_action is not None
    torch.testing.assert_close(first_action, torch.tensor([53.0, 53.0]))
